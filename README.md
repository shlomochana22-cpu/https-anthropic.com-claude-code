...
work-plan-p0-p2.md            # תוכנית עבודה מתועדפת לשלבים P0+P1+P2 (פעיל)
claude-chrome-master.md       # פרומפט-על אחד רציף (סריקה + כל התיקונים) להדבקה ל-Claude בכרום
claude-chrome-audit-mission.md # משימת סריקה טכנית מלאה ל-Claude בכרום (להריץ ראשונה)
claude-chrome-prompt.md       # פרומפטים מפורטים לביצוע התיקונים, בלוק לכל משימה
city-blocks.md                # GEO Box + קישורים פנימיים מוכנים ל-10 הערים המובילות
...
- בלוקי הערים (city-blocks.md): אמת שכל קישור פנימי נפתח (לא 404) לפני פרסום. ה-slug של הערים באתר אינו עקבי.

## מחיר אחיד

המחיר ההתחלתי בכל החבילה אוחד ל-449 ש"ח (מצבר רגיל), מצבר סטארט סטופ החל מ-749 ש"ח. כל הקבצים עודכנו בהתאם.
