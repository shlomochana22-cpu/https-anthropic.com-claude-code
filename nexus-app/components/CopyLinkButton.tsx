...
export function CopyLinkButton({ eventId, promoter }: { eventId: string; promoter?: string }) {
  const [copied, setCopied] = useState(false);
  const copy = () => {
    const ref = promoter ? encodeURIComponent(promoter) : "promoter";
    const link = `https://nexusevents.co.il/events/${eventId}?ref=${ref}`;
  ...
  return (
    <button onClick={copy} className="w-full bg-surface-container-high border border-white/10 text-primary py-sm rounded-lg text-label-md flex items-center justify-center gap-2 active:scale-95 transition-transform hover:border-primary-fixed/40">
      ...
      {copied ? "הלינק הועתק!" : promoter ? `העתק לינק ל${promoter}` : "העתק לינק"}
    </button>
  );
}
