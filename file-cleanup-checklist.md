...
עדכון: התוסף WPCode הושבת זמנית כי הסניפט הזדוני לא ניתן להסרה דרך הממשק (כל עמודי הניהול של WPCode מחזירים 403/500, ככל הנראה שובש). מקור אחד של הלואדר היה תיבת ה-Footer (כבר נוקתה), ומקור שני הוא סניפט נפרד ששמור ב-WPCode (post_type=wpcode).

בגישת אחסון/DB:
- נקה את הסניפט הזדוני מ-wp_posts (WHERE post_type='wpcode' שמכיל __performance_optimizer_v6 / atob). הראה לי לפני מחיקה.
- אחרי הניקוי, אם רוצים להחזיר את WPCode לשימוש, מומלץ למחוק ולהתקין אותו מחדש נקי (כי הממשק היה שבור), ולוודא שתיבת ה-Header מכילה רק את הסכמה הנקייה.
- את הסכמה הנקייה לדף הבית נתקין מ-homepage-schema.html.
